...
It is recommended to use case_sensitive=>1 in new()
and print_to=>\*STDOUT in output() for best perfomance.

NOTE: you can safely write 
  my $template = HTML::Template->new( ... options ...)
  with HTML::Template::Pro, because in absence of original HTML::Template
  HTML::Template::Pro intercepts its calls.

  You also can use both modules and safely mix their calls.
...
There is described a perl interface of HTML::Template::Pro and HTML::Template.
...
I<WARNING>: filters are not supported in HTML::Template::Pro.
...
the following options are not supported in HTML::Template::Pro:
 ...
 filter, vanguard_compatibility_mode.
...
explicitly call new() with die_on_bad_params => 0, strict => 0.
